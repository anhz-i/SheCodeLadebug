...
public class ChangePass extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        User us;
        if (!crpass.trim().equals(user.getPassword().trim())) {
            request.setAttribute("error", "Current password errors!" + user.getPassword());
            request.getRequestDispatcher("view/changepass.jsp").forward(request, response);
        } else {
            if (pass.trim().equals(cfpass.trim())) {
                try {
                    u.Update(user.getName(), user.getEmail(), pass);
                    us = u.getUser(user.getEmail(), pass);
